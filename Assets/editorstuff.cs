...
public class GridEditor : Editor {
	...
	void OnSceneGUI(){
		...
		// dragging action begin
		if(e.isMouse &&e.type == EventType.MouseDown && e.button == 0 && grid.draggable){
		...
		if(e.isMouse &&e.type == EventType.MouseUp && e.button == 0 && grid.draggable){
			e.Use();
			FillArea(mouseBeginPos,mousePos);
			Debug.Log ("Begin: " + mouseBeginPos.ToString() + "  End: " + mousePos.ToString());
			mouseBeginPos = new Vector3(0,0,0);
		}
		// dragging action end
		...
		if(e.isMouse && e.type == EventType.MouseUp || e.isKey && e.type == EventType.KeyUp){
